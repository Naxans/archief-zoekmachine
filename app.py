...
if submit_button:
    if not onderzoeksvraag.strip():
        ...
        with st.spinner("Stap 1/3: Inhoudsopgave (Google Sheet) scannen..."):
            ...
            # AANGEPAST: Slimme directe trefwoord-matching in de hele rij
            zoekwoorden = [w.lower() for w in onderzoeksvraag.split() if len(w) > 2]
            for row in data:
                doc_id_val = str(row.get('Document_ID', '')).strip()
                rij_tekst = " ".join([str(val) for val in row.values()]).lower()
                
                for woord in zoekwoorden:
                    if woord in rij_tekst:
                        gekozen_id = doc_id_val if doc_id_val else f"SINGLE_{row.get('Bestandsnaam', '').strip()}"
                        if gekozen_id not in geselecteerde_doc_ids:
                            geselecteerde_doc_ids.append(gekozen_id)
                        break

            # Zoek via Gemini voor aanvullende Document_ID's
            ...
            for d_id, d_info in dossier_samenvattingen.items():
                ...
                best_str = ", ".join(d_info["Bestanden"])
                regel = f"Document_ID: {d_id} | Bestanden: {best_str} | Datum: {d_info['Datum']} | Personen: {pers_str} | Onderwerp: {ond_str}"
            ...
            filter_prompt = f"""
Jij bent hoofdarchivaris. Hieronder staat een overzicht van de unieke dossiers en bestanden in ons archief:
...
INSTRUCTIES:
1. Selecteer alle dossiers (Document_ID's) of losse bestanden (SINGLE_...) die relevante informatie bevatten voor de vraag.
2. Let extra goed op specifieke modelnamen (zoals Vedette), merknamen, personen en document-typen (zoals PDF's of catalogi).
3. Geef maximaal {max_dossiers} meest relevante ID's terug.

Geef UITSLUITEND de exacte Document_ID's of SINGLE_ bestandsnamen terug gescheiden door komma's. Geen extra tekst of uitleg.
"""
            ...
            try:
                ...
                for ai_id in ai_ids:
                    if ai_id not in geselecteerde_doc_ids:
                        geselecteerde_doc_ids.append(ai_id)
            except Exception as e:
                st.warning(f"Automatische AI-selectie overgeslagen: {e}")

        if not geselecteerde_doc_ids:
            st.warning("Geen relevante documenten gevonden op basis van de zoekopdracht.")
            st.stop()

        # STAP 1.5: Verzamel ALLE bestanden die bij de geselecteerde ID's horen
        eind_bestanden_lijst = []
        for row in data:
            doc_id = str(row.get('Document_ID', '')).strip()
            b_naam = str(row.get('Bestandsnaam', '')).strip()
            single_id = f"SINGLE_{b_naam}"

            if any(g_id.lower() in [doc_id.lower(), b_naam.lower(), single_id.lower()] for g_id in geselecteerde_doc_ids):
...
Beantwoord onderstaande onderzoeksvraag grondig op basis van ALLE meegeleverde originele archiefstukken (afbeeldingen EN PDF-documenten).

ONDERZOEKSVRAAG: {onderzoeksvraag}

INSTRUCTIES VOOR JE RAPPORT:
1. Neem alle informatie over het onderwerp over uit zowel de afbeeldingen als de PDF-documenten.
2. Vermeld expliciet alle bronnen en bestandsnamen (bijv. 'catalogus.pdf' of 'foto.jpg') waarin de informatie te vinden is.
3. Als er meerdere bedrijven of modellen met dezelfde naam zijn, leg dit verschil dan uit.
4. Trek een heldere conclusie.
"""
            ]
